...
public class PrismExporter {
    ...
    public PrismExporter(String modelName) {
    ...
    private void processTransition() {
        ...
        try {
            ...
            VertexProvider<TransitionVertex> vertexProvider = (s, map) -> {
                int currentID;
            ...
            EdgeProvider<TransitionVertex, EdgeTransitionGraph> edgeProvider = (v1, v2, s2, map) -> {
                String label = (map.get("label")).toString();
                float weight = GraphsCollection.getInstance().getReactionWeight(label);
                return new EdgeTransitionGraph(label,weight);
            };

            ComponentUpdater<TransitionVertex> vertexUpdater = (vertex, map) -> {
                Attribute label = map.get("label");
                Attribute propertiesAttribute = map.get("properties");
